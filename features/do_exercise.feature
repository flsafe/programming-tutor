Feature: Do exercise
	...
	Scenario: The user submits a solution
		...
		And the exercise "RemoveChar" has the associated solution template and unit test
		And I am viewing the tutor page for "RemoveChar"
		When I fill in the text editor with "void remove_char(char c, char str[]){ write_index = 0; int read_index  = 0; char curr_char; do{ curr_char = str[read_index]; if(curr_char != c){ str[write_index] = str[read_index]; write_index++;} read_index++; }while(curr_char);}"
		And I press "Submit"
