...
describe GradeSheetsController do
  ...
  describe "get show_user_grade_sheet" do
    ...
    it "retrieves the associated exercise" do
      Exercise.should_receive(:find).with(@stub_exercise.id.to_s).and_return(@stub_exercise)
    ...
    it "retrevies the lastest grade sheet for the given exercise_id" do
      Exercise.stub(:find).and_return(@stub_exercise)
      GradeSheet.should_receive(:find).with(:first, :conditions=>['exercise_id=? AND user_id=?', @stub_exercise.id, @stub_current_user.id], :order=>'created_at DESC')
      get :show_user_grade_sheet, :id=>@stub_exercise.id.to_s
    end
