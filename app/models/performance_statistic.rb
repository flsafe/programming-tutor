class PerformanceStatistic < ActiveRecord::Base
  ...
  def self.place_stat(user_id, exercise_id, stat_name, value)
    PerformanceStatistic.clear_slot(user_id, exercise_id, stat_name) #We assume the user can only do one exercise at a time, so clear the stats from the last exercise that was completed
  ...
  def self.clear_slot(user_id, exercise_id, stat_name)
    PerformanceStatistic.delete_all :user_id=>user_id, :exercise_id=>exercise_id, :name=>stat_name
  end
