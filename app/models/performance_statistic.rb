class PerformanceStatistic < ActiveRecord::Base
  ...
  def self.place_stat(user_id, exercise_id, name, value)
    PerformanceStatistic.clear_slot(user_id, name) #We assume the user can only do one exercise at a time, so clear the stats from the last exercise that was completed
  ...
  def self.clear_slot(user_id, name)
    PerformanceStatistic.delete_all :user_id=>user_id, :name=>name
  end
